...
describe('NavbarComponent', () => {
  ...
  it('links sections to the landing page in the expected navigation order', () => {
    const links = navLinks()
    const priceUrl = new URL(linkByLabel('Ceník').href)
    const certificatesUrl = new URL(linkByLabel('Certifikáty').href)
    const photoUrl = new URL(linkByLabel('Foto').href)
    const videoUrl = new URL(linkByLabel('Video').href)
    ...
    expect(links.map(link => link.textContent?.trim()))
      .toEqual(['Domů', 'Ceník', 'Certifikáty', 'Region', 'Novinky', 'Foto', 'Video', 'Recepty', 'Kontakt'])
    expect(priceUrl.pathname).toBe('/')
    expect(priceUrl.searchParams.get('locale')).toBe('cs')
    expect(priceUrl.hash).toBe('#cenik')
    expect(certificatesUrl.pathname).toBe('/')
    expect(certificatesUrl.searchParams.get('locale')).toBe('cs')
    expect(certificatesUrl.hash).toBe('#certifikaty')
    expect(photoUrl.pathname).toBe('/')
    expect(photoUrl.searchParams.get('locale')).toBe('cs')
    expect(photoUrl.hash).toBe('#foto')
    expect(videoUrl.pathname).toBe('/')
    expect(videoUrl.searchParams.get('locale')).toBe('cs')
    expect(videoUrl.hash).toBe('#video')
